...
Their ``practical finance'' approach makes it reasonably feasible for an ordinary person to calculate an asset allocation very close to the model's optimum, at least with the aid of spreadsheet software.
...
This same reasoning speaks to the long-standing ``annuity puzzle'': consumers who value their wealth stock will naturally resist converting it entirely into an income stream, however actuarially favorable.
...
In particular, it is critical to be wary of assumptions that lead a model to recommend that wealth be drawn down to zero.
...
For financial advisers who want to begin exploring life-cycle modeling and how they can incorporate it into their own advice, we humbly suggest looking into \href{https://github.com/econ-ark/HARK}{\texttt{HARK}}, our open source toolkit for solving and simulating dynamic stochastic optimization problems.
%\texttt{HARK} uses Python, and the latest version of the package can be installed using \texttt{pip} or \texttt{conda}.
We are constantly developing and improving our software, and have composed helpful documentation and tutorials, which can be installed locally with \texttt{HARK} or viewed through our \href{https://econ-ark.org}{website}.
