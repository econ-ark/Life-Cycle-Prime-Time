...
\cite{Choi2025} use the solution to the life-cycle problem for a wide array of parameters to produce \textit{approximately optimal} rules for allocating one's wealth between risky and riskless assets.
...
Likewise, it is worth examining whether clients have adequate access to \textit{liquid} wealth; a model that tracks only total net worth may recommend a spending path that is infeasible given the actual composition of the balance sheet.
...
Because the wealth-in-utility parameter $\delta$ can be informed by asking clients directly about their comfort with drawing down savings, the framework lends itself naturally to personalized advice-- whether delivered by a human planner or an automated tool.
