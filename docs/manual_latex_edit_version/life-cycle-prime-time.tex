...
\noindent Christopher Carroll (\texttt{ccarroll@jhu.edu}) is a Professor of Economics at the Krieger School of Arts and Sciences, Johns Hopkins University (Baltimore, MD)

\vspace{1cm}

\noindent Alan Lujan (\texttt{alujan@jhu.edu}) is a Lecturer and Program Coordinator for Applied Economics, Krieger School of Arts and Sciences, Johns Hopkins University (Washington, DC)

\vspace{1cm}

\noindent Matthew N. White (\texttt{mnwhite@gmail.com}) is the Primary Developer for Econ-ARK (Baltimore, MD) and the corresponding author for this paper
...
But academics and advisors know that the advice about both saving and portfolio choice provided by standard life-cycle models is deeply problematic.
...
\begin{itemize}
	...
	\item Accounting for ``soft data'' from surveys about consumer preferences and expectations is critical for building a quantitative model that gives credible advice. Likewise, advisers must consider whether the model should optimize on clients' subjective beliefs or their own expert knowledge.
...
An enormous academic literature has followed their pioneering work,
...
Indeed, the subtitle of a recent paper by \cite{Daga_2023} captures the current state of affairs nicely: ``Why Practitioners Have Not Adopted the Lifecyle Model -- Yet''; see also \cite{Idzorek_2024}, who comment on practitioners' disconnect between intratemporal portfolio optimization and intertemporal life-cycle planning.
...
Using life-cycle models to inform their advice is not wholly foreign to financial advisers,\footnote{E.g.,\ \cite{Lanski2022} provide a case study for their experience in developing and iterating on a life-cycle model to design a target date glidepath.} but it is not yet commonplace.
...
The incorporation of realistic descriptions of these complexities makes computation of the mathematically optimal solution to the consumer's problem surprisingly difficult.
...
Our proposed solution to this conundrum involves a small twist to the old idea (from \cite{WhyDoTheRich}) of incorporating wealth \textit{directly} in the utility function.
Rather than indirectly valuing wealth because it will finance future consumption and act as a buffer against risks both known and unknown, consumers in this model derive utility from the mere fact that they hold wealth.
This modeling choice could represent a reduced form for other motivations--- indeed, that was the thesis of \cite{WhyDoTheRich}.
As we discuss further below, retirees responding to the Survey of Consumer Finances most commonly cite ``Liquidity / The Future'' as their most important reason for saving.
These responses might signal that the forms of uncertainty that we can measure -- such as actuarial calculations about nursing home expense risks -- constitute only a fraction of the risks retirees might worry about.
...
Directly incorporating wealth into the utility function may serve as a tractable proxy for this profound uncertainty aversion.


\section{Recent Developments in the Literature}

We begin with a brief literature synopsis before proceeding in Section~\ref{models} to a set of models that illustrate the difficulty of matching empirical facts with the life-cycle approach.

\subsection{Survey Data on Expectations and Preferences}

A key academic development has been a new openness to the idea that people's beliefs and preferences can be probed by \textit{asking them} about their beliefs and preferences.
...
Among retirees, one answer dominates the rest: ``Liquidity / The Future.''

A natural interpretation of the importance consumers put on ``liquidity'' is that precautionary saving motives matter for many households, highlighting the need for computational advancements in solving such models.
...
Concretely, many people think that investment in stocks is a lousy deal, yielding a low return with high risk, so it's no mystery why such people do not invest.\footnote{If the modeler is willing to assert that consumers have mistaken beliefs that cause them to make suboptimal choices, the advice the model gives will differ from the pattern of measured behavior. This could justify the model in recommending, for example, greater investment in risky assets than consumers tend to choose on their own. We compromise by adopting a believed equity premium of 0.03, which is lower than the historical average. Even lower beliefs would reduce the estimated risk aversion coefficient. Alternatively, incorporating plausible estimates of behavioral frictions of equity market participation can also explain low rates of stock-holding, as in \cite{Choukhmane2024}.}
...
The remarkable advance of computational power has made it possible to compute a credible answer to the question, ``What saving and portfolio choices are truly mathematically optimal?'' in a context in which (at least some of) the key complexities are properly represented.
In Section~\ref{models}, we provide a formal description of the mathematical and computational structure of such optimizing models, beginning with the Life~Cycle~Portfolio model, which calculates optimal saving and optimal portfolio shares over the life cycle.
...
We next modify the model by adding a bequest motive, because the literature has extensively explored whether such a motive could explain the drawdown failure (see \cite{DeNardi2016d}).
In Section~\ref{estimation}, we find that in order for the bequest motive to explain the drawdown failure, the strength of that motive has to be implausibly strong--- so much so that even early in their working lives, the primary motivation for saving is the accumulation of a bequest (rather than, say, to sustain one's own consumption in retirement).
...
The academic literature on life-cycle modeling is vast, and we cannot hope to do it justice.
...
$\aNrm$ & end-of-period assets normalized by permanent income \\
...
These values are somewhat low compared to the economics literature (in which $\Rfree = 1.02$ and $\eprem = 0.06$ are common), but from personal correspondence with financial planners, our understanding is that the profession tries to be conservative about projections of their clients' portfolio returns.
...
Exhibit~\ref{most-important-reason} presents the responses to this question for college-educated households older than age 70 from the 1995 to the 2022 waves of the SCF.
...
\begin{align}
    \uFunc(\cNrm,\lqdt) & = \frac{\left(
        ...
        \right)^{1-\CRRA}}{1-\CRRA}
\end{align}
where $\lqdt$ captures the $\lqdt$iquidity services provided by money-holding.
...
\cite{O_Hara_2015} incorporate an additively separable utility of bequest like the one described above, and like our models it incorporates both permanent and transitory shocks to income.
...
\cite{Lanski2022} describes how the authors used the canonical life-cycle consumption and portfolio-choice model to improve their target date fund's glidepath.
Their article focuses on the iterative process and challenges faced by investment advisors when using the model, rather than trying to rectify the drawdown failure.

\cite{Choi2025} use the solution to the life-cycle problem for many parameter combinations to produce \textit{approximately optimal} rules for allocating one's wealth between risky and riskless assets.
That is, each individual has their own unique preferences, expected income growth profile, income risk, etc, but the solution to the life-cycle problem applies to only \textit{one} particular set of such parameters-- and the vast majority of people are not equipped to solve the intertemporal optimization problem for their own circumstances.
Their ``practical finance'' approach makes it reasonably feasible for an ordinary person to calculate an asset allocation not too far from the model's optimum, at least with the aid of spreadsheet software.
...
Exhibit~\ref{parameters} shows the results of our estimation exercise, and the fit of the three estimated models is plotted below, in Exhibit~\ref{medwealth} and Exhibit~\ref{medshare}.
...
The ``criterion'' column of Exhibit~\ref{parameters} lists the minimum value that the objective function achieves for each model-- the smallest weighted squared distance between simulated and empirical moments.
...
To try to match the empirical wealth trend (red dashed line in Exhibit~\ref{medwealth}), which holds steady at a high wealth-to-income ratio at older ages, the LCP model (solid blue line) exceeds observed wealth accumulation through the working life.
...
In Exhibit~\ref{nobequest}, we reproduce the median wealth moment fit for the Warm-Glow Portfolio model from Exhibit~\ref{medwealth} and add a new model moment plot for agents whose bequest motive has been turned off.
...
Unsurprisingly, the drawdown failure returns in Exhibit~\ref{nobequest} because retirees have no incentive to retain wealth as the likelihood of mortality rises.
...
And as discussed above, the bequest motive is inconsistent with an investment advisor's fiduciary duty \textit{to the client}, as in the case of (e.g.) the sponsor of a 401(k) plan.
...
Moreover, because the CRRA parameter doesn't need to be so high, the WIUF model can somewhat better match the target risky assets share moments (red dashed line in Exhibit~\ref{medshare}).
...
Risky asset shares for both sets of beliefs are plotted on Exhibit~\ref{subjbeliefs}.
Unsurprisingly, agents would want to hold a considerably greater share of their wealth in risky assets if they learned that stock returns have much lower variance than they previously believed.
...
We argue that including wealth directly in consumers' utility function can serve as a computationally tractable shorthand for the unenumerable risks that retirees might be concerned about.
This same reasoning speaks to the long-standing ``annuity puzzle'': consumers who value their wealth stock resist converting it entirely into an income stream, however actuarially favorable.
The wealth-in-utility framework can help determine not \textit{whether} to annuitize, but \textit{how much}.

Modelers must always make difficult choices about which features to include to bolster a model's realism and which to forgo to limit computational burden.
In the context of providing advice as a financial planner, we recommend thinking carefully about the simplifying assumptions that your model makes, including those that were made ``silently'' or by defaulting to what you have seen or done before.
In particular, a model that does not have plausible explanations for why people do not want to draw their wealth down to zero should not be taken seriously as providing normative advice.
Likewise, practitioners should ask whether clients have adequate access to \textit{liquid} wealth; a model that tracks only total net worth may recommend a spending path that is infeasible given the actual composition of the balance sheet.
If your clients would resist this consumption advice later in life, then the portfolio allocations your model provided earlier in life were \textit{also incorrect}.

Much more remains to be done to improve the models further; for example, a question of great practical importance that is now just at the edge of possibility of being computationally solved is to calculate the implications of non-financial (principally, housing) wealth for optimal financial choice.
The academic literature is only now reaching the point at which it may be possible to answer questions like, ``If I own a house, how should I modify my spending and portfolio plans to take that into account?''
We do know the \textit{direction} of the effect (\cite{kimballStandardRA}),
...
Not only would academics have the satisfaction of knowing that they had finally come close to fulfilling the vision of Modigliani and Brumberg 70 years ago.
Financial analysts could also sleep more soundly in the knowledge that the advice they were giving was what many people probably think it already is: The adaptation to the client's particular circumstances of the advice that is the best that can be delivered by the latest high-tech computational optimization tools.
...
Because the wealth-in-utility parameter $\delta$ governs the trade-off between utility from consumption and utility from holding wealth, it can be calibrated by asking clients directly about their comfort with drawing down savings.
Advisers and automated tools alike could use this as a lever for personalized advice, tailoring the model to each client's preference for maintaining a buffer stock.

For financial advisers who want to begin exploring life-cycle modeling and how they can incorporate it into their own advice, our open source toolkit for solving and simulating dynamic stochastic optimization problems (\href{https://github.com/econ-ark/HARK}{\texttt{HARK}}) is available and aims to be usable even by those who are not experts in dynamic stochastic optimization problems.
%\texttt{HARK} uses Python, and the latest version of the package can be installed using \texttt{pip} or \texttt{conda}.
We are constantly developing and improving our software, and have composed comprehensive documentation and tutorials, which can be installed locally with \texttt{HARK} or viewed through our \href{https://econ-ark.org}{website}.
The \href{https://github.com/econ-ark/life-cycle-prime-time}{GitHub repository} for this paper also includes the particular code we used to perform the estimations here, as well as an interactive Jupyter notebook to explore our results.
...
\appendix
\section{Online Appendix: Solution Methods}
