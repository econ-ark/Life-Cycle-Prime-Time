...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/WealthMomentFit-86bf7f8de1e4e0cab6323cbc126e3f54.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/StrongBequestMotive-58abbedde72fa1826117fe13ba12e73a.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/ShareMomentFit-e25041a5096dcf653d7bf43566eb63ba.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/StockBeliefsShare-90d1b934c6651f923a0447d115e40327.pdf}
...
The expression in square brackets is raised to a negative power, which would yield a complex result with a negative input, hence we need only to consider values of $\chi \in (0, (1 -\delta)/\delta)$. These $\chi$ values represent the \textit{range} of the function that maps from $\omega$ to $\chi$, and we want our constructed approximation to cover as much of that range as possible. To do so, we will use the logit transformation to map from an auxiliary variable $z \in \R$ to $\chi \in (0, (1 -\delta)/\delta)$:
...
\item For each $\omega$ in that vector, compute $\chi = f(\omega)$, then multiply by $\aNrm$ to recover the optimal $\cNrm$.
...
Numerically solving the wealth-in-utility model is thus \textit{barely} more computationally burdensome than solving the basic life-cycle model, despite the considerably more complex preferences.
